...
def load_wav_for_rms(path):
    ...
    try:
        ...
        # MP3: soundfile может не поддерживать. Пробуем pydub (как в robot_mouth_talk.py).
        if path.lower().endswith(".mp3"):
            try:
                ...
                return samples.astype("float32"), audio.frame_rate, len(samples) / float(audio.frame_rate)
            except Exception:
                return None, 0, 0.0
        return None, 0, 0.0
...
class RobotMouthTalkNode:
    ...
    def _play_and_animate(self, audio_path):
        ...
        while rospy.is_shutdown() is False:
            ...
            # Уровень рта: рисуем рот по RMS из текущего воспроизводимого файла.
            # Это делает поведение таким же, как в robot_mouth_talk.py (ручной режим).
